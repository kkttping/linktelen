...
export default function AboutContact() {
    ...
    useEffect(() => {
        ...
        let script = document.createElement('script');
        script.type = 'text/javascript';
        script.src = 'https://www.bing.com/api/maps/mapcontrol?key=Alq8m5uHndfHyzsWSTgXv1EAX9xc8wQACj9kS0r_5-xWnUdWAZjQ7q4SSEwCAgEo';
        document.getElementById('root').appendChild(script);
        script.onload = () => {
    ...
    useEffect(() => {
        ...
        if (screenWidth <= 1400) {
            setScale(((screenWidth / 1920) * 1.2 + .05) * 0.8);
        } else {
            setScale(.9);
        }
        if (screenWidth <= 640) {
            ...
            <div className='top_backg'>
                ...
                <div className='img_box'>
                    <div className='title' style={{ opacity: isphone ? '0' : '1' }}>
                        <div className='tit'>Global Operation</div>
                        <div className='info'>
                            {info?.info1}
                        </div>
                    </div>
                    <div className='title t_phone' style={{ display: isphone ? 'block' : 'none' }}>
                        <div className='tit'>Global Operation</div>
                        <div className='info'>
                            {info?.info1}
                        </div>
                    ...
                    <div className='box' >
                        <div className='icon_phone' style={{ opacity: isphone ? '1' : '0' }}>
                            {searchKeyWords('USA', mapList)?.Office && <div className='item1' >
                                <div className='item_bg' style={{ backgroundImage: `url(${ConstValue.url + "assets/" + searchKeyWords('USA', mapList)?.top_icon})` }}></div>
                                <div className='item_text'>{searchKeyWords('USA', mapList)?.text_top}</div>
                            </div>}
                            {searchKeyWords('Singapore', mapList)?.Office && <div className='item1' >
                                <div className='item_bg' style={{ backgroundImage: `url(${ConstValue.url + "assets/" + searchKeyWords('Singapore', mapList)?.top_icon})` }}></div>
                                <div className='item_text'>{searchKeyWords('Singapore', mapList)?.text_top}</div>
                            </div>}
                            {searchKeyWords('China', mapList)?.Office && <div className='item1' >
                                <div className='item_bg' style={{ backgroundImage: `url(${ConstValue.url + "assets/" + searchKeyWords('China', mapList)?.top_icon})` }}></div>
                                <div className='item_text'>{searchKeyWords('China', mapList)?.text_top}</div>
                            </div>}
                            {searchKeyWords('Malaysia', mapList)?.Office && <div className='item1' >
                                <div className='item_bg' style={{ backgroundImage: `url(${ConstValue.url + "assets/" + searchKeyWords('Malaysia', mapList)?.top_icon})` }}></div>
                                <div className='item_text'>{searchKeyWords('Malaysia', mapList)?.text_top}</div>
                            </div>}
                        </div>
                        <div className='lable_top' style={{ transform: isphone ? 'scale(0)' : 'scale(1)' }}>
                            ...
                            <div className={isHow ? 'def' : 'show'}>
                                <div className='defut'><p>Move to icon</p>
                                    <p>Learn more</p></div>
                                {/* {qien(mapList?.[0]?.text_top) ?? <><p>Move to icon</p>
                                    <p>Learn more</p></>} */}
                            </div>
                        ...
                        <div className='boder' style={{ transform: `scale(${scale1})`, backgroundSize: isphone ? '0%' : 'cover' }}>
                            <div className='icon' style={{ opacity: isphone ? '0' : '1' }}>

                                {searchKeyWords('USA', mapList)?.Office && <div className='item1' onMouseLeave={() => { setShowText(); setIsShow(false) }} onMouseEnter={() => { setShowText(searchKeyWords('USA', mapList)?.text_top); setIsShow(true) }}>
                                    <div className='item_bg' style={{ backgroundImage: `url(${ConstValue.url + "assets/" + searchKeyWords('USA', mapList)?.top_icon})` }}></div>
                                    <div className='item_text'>{searchKeyWords('USA', mapList)?.Office}</div>
                                </div>}
                                {searchKeyWords('Singapore', mapList)?.Office && <div className='item2' onMouseLeave={() => { setShowText(); setIsShow(false); setIsShow(false) }} onMouseEnter={() => { setShowText(searchKeyWords('Singapore', mapList)?.text_top); setIsShow(true) }}>
                                    <div className='item_bg' style={{ backgroundImage: `url(${ConstValue.url + "assets/" + searchKeyWords('Singapore', mapList)?.top_icon})` }}></div>
                                    <div className='item_text'>{searchKeyWords('Singapore', mapList)?.Office}</div>
                                </div>}
                                {searchKeyWords('China', mapList)?.Office && <div className='item3' onMouseLeave={() => { setShowText(); setIsShow(false) }} onMouseEnter={() => { setShowText(searchKeyWords('China', mapList)?.text_top); setIsShow(true) }}>
                                    <div className='item_bg' style={{ backgroundImage: `url(${ConstValue.url + "assets/" + searchKeyWords('China', mapList)?.top_icon})` }}></div>
                                    <div className='item_text'>{searchKeyWords('China', mapList)?.Office}</div>
                                </div>}
                                {searchKeyWords('Malaysia', mapList)?.Office && <div className='item4' onMouseLeave={() => { setShowText(); setIsShow(false) }} onMouseEnter={() => { setShowText(searchKeyWords('Malaysia', mapList)?.text_top); setIsShow(true) }}>
                                    <div className='item_bg' style={{ backgroundImage: `url(${ConstValue.url + "assets/" + searchKeyWords('Malaysia', mapList)?.top_icon})` }}></div>
                                    <div className='item_text'>{searchKeyWords('Malaysia', mapList)?.Office}    </div>
                                </div>}
                            </div>
                            <div className='point'>
                                {searchKeyWords('USA', mapList)?.Office && <div className='item1'>
                                ...
                                {searchKeyWords('China', mapList)?.Office && <div className='item2'>
                                    <div className='point2'></div>
                                    <div className='area2'></div>

                                </div>}
                                {(searchKeyWords('Singapore', mapList)?.Office || searchKeyWords('Malaysia', mapList)?.Office) && <div className='item3'>
                                    {searchKeyWords('Malaysia', mapList)?.Office && <div className='point3'></div>}
                                    {searchKeyWords('Singapore', mapList)?.Office && <div className='point4'></div>}
                                            ...
                                            {mapList?.map((item, index) => {
                                                ...
                                                return (<Col md={12} xs={24} xxl={12}>
                                                    ...
                                                    <div className={'info_box '}>
                                                        ...
                                                        <div className='office'>· {mapList[index]?.Office}</div>

                                                        <div style={{ marginBottom: '16px' }} >{mapList[index]?.Address}</div>
